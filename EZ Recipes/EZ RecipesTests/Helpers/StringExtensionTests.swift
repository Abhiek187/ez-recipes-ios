...
@Suite struct StringExtensionTests {
    @Test("Converts summaries to markdown", arguments: [
        (
    ...
    ]) func convertsSummariesToMarkdown(input: String, expectedOutput: String) {
        #expect(input.htmlToMarkdown == expectedOutput)
    }
    
    @Test func convertsRegularHTMLToMarkdown() {
        let html = """
